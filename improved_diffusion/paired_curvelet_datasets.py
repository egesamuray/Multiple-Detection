...
class PairedCurveletH5(Dataset):
    ...
    """
    def __init__(self, h5_path: str, input_key: str = "srmemult", target_key: str = "prim",
                 j: int = 1, image_size: Optional[int] = 256,
    ...
    def __getitem__(self, idx: int):
        ...
        X = packed_tgt[0]              # (Wj,Nj,Nj) since C=1
...
def load_data_curvelet_paired_h5(h5_path: str, input_key: str, target_key: str,
                                 batch_size: int, j: int, image_size: int,
                                 angles_per_scale=None, stats=None,
                                 deterministic: bool=False, num_workers: int=0,
                                 limit: Optional[int]=None):
    ds = PairedCurveletH5(h5_path=h5_path, input_key=input_key, target_key=target_key,
...
@torch.no_grad()
def curvelet_stats_hdf5(h5_path: str, target_key: str="prim", j: int=1,
                        angles_per_scale=None, image_size: Optional[int]=256, limit: Optional[int]=None):
    ...
    for i in range(N):
        ...
        if sum_c is None:
            sum_c = flat.sum(dim=1); sumsq_c = (flat**2).sum(dim=1)
        else:
            sum_c += flat.sum(dim=1); sumsq_c += (flat**2).sum(dim=1)
        count += flat.size(1)
    mean = sum_c / count
