...
def main():
    ...
    model, diffusion = create_model_and_diffusion(
        task="curvelet", **args_to_dict(args, model_and_diffusion_defaults(task="curvelet").keys())
    )
    ...
    data = load_data_curvelet_paired_h5(
        h5_path=args.h5_path, input_key=args.input_key, target_key=args.target_key,
...
def create_argparser():
    defaults = dict(
        # data/model
        h5_path="", input_key="srmemult", target_key="prim",
        j=1, angles_per_scale="8,16,32,32", large_size=256, color_channels=1,
        diffusion_steps=256,
        # training
        batch_size=8, lr=1e-4, weight_decay=0.0, lr_anneal_steps=0, max_training_steps=500000,
        microbatch=-1, ema_rate="0.9999", log_interval=10, save_interval=10000,
        resume_checkpoint="", use_fp16=False, fp16_scale_growth=1e-3,
        # misc
        logdir="results/curvelet_J1/cond",
        stats_npz="", limit=-1,
    )
